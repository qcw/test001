...
cmd="
python -m torch.distributed.launch --nproc_per_node=4 run_tasks.py --model ${model} --task ${task}
    --num_landmarks ${lms}   --conv_kernel_size ${k_conv}  --window_size ${wsize} ${flags}
    --weight_decay 1e-2  --num_layers ${layers}  --chk_path ${chkpath} --max_seq_len 16384
...
runexp    0   pathfinder128-curv_contour_length_14  attention_SKT    "LRA_chks/path_dynamic_v77_softmax_d_softmax_n_1_1017"    1   256    -1      8     1e-2   0.0 1234 
